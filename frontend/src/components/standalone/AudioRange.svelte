<script lang="ts">
	...
	import { audioPlayerState, audioStore, resetTrackTimer, userTapped } from '../../stores/AudioPlayerStore';
	...
	$: {
		if (rangeInput && duration > 0) {
			...
			if (averageColor) {
				...
				rangeInput.style.setProperty('--accent-faded', averageColor.replace('rgb', 'rgba').replace(')', ', 0.3)'));
			}
			rangeInput.style.setProperty('--border-radius', roundedEdges ? '5px' : '0px');
		}
	}
</script>
...
{#if $userTapped}
	<div class="wrapEntireRange" class:smallRange={!showTrackTime}>
		{#if $audioPlayerState !== 'Loading'}
			{#if duration >= 0}
				<div in:fade={{ duration: 150 }} out:fade={{ duration: 150 }} class="contentWrapper">
		...
		{:else}
			<div in:fade={{ duration: 150 }} out:fade={{ duration: 150 }} class="contentWrapper">
				<div class="wrapLoaderJ">
					<AudioLoader height={'7px'} backgroundColor={'transparent'}/>
				</div>
			</div>
		{/if}
	</div>
