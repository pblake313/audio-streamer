<script lang="ts">
    ...
    import { selectedBeat, selectNewBeat } from "../../../stores/AudioPlayer/selectedBeatStore";
    import { audioPlayerState, pauseTrack, playTrack, useAutoPlay, userTapped } from "../../../stores/AudioPlayerStore";
    ...
    import AudioLoader from '../../reusable/Loaders/AudioLoader.svelte';
    ...
    $: sortedDestinations = [...(beat.futureDestinations || [])].sort((a, b) => a.localeCompare(b));
...
<button type="button" class="trackButton" class:evenButton={isEven} on:click={selectAndPlay}>
    ...
    <div class="artworkWrapper">
        ...
        {#if $selectedBeat?.beatTitle === beat.beatTitle && $userTapped}
            ...
            <div class="playingOverArt selTrackItem" class:hiderJ={['Paused', 'Idle'].includes($audioPlayerState)} class:goHide={$audioPlayerState === 'Paused' || $audioPlayerState === 'Idle'}>
                <div class="wplay">
    ...
    <div class="allButArt">
        <div class="titleWrap">
            ...
            <div class="flexDaysOld">
                ...
                <div class="mobTagsFlex">
                    {#if beat.customTag}
                        <BeatTag tagColor={`#${beat.customTagColor}` || '#353535'} tagText={beat.customTag} tagTextColor={'#222222'}></BeatTag>
                    {/if}
                    {#if beat.tagOne}
        ...
        <div class="taggerflexer">
            ...
            {#if beat.customTag}
                <BeatTag tagColor={`#${beat.customTagColor}` || '#353535'} tagTextColor={beat.customTagColor ? '#222222' : '#f7f7f7'} tagText={beat.customTag}></BeatTag>
            {/if}
            {#if beat.tagOne}
                <BeatTag tagText={beat.tagOne}></BeatTag>
            {/if}
        ...
        <div class="moreInfoC">
            ...
            <div class="wrappamo">
                <button class="moreIconButton" on:click={selectMoreBeat}><MoreIcon color={'#fff'}></MoreIcon></button>
        ...
        <div class="editBeatInfoBox">
            ...
            <div class="beatRatingFlex">
                <div class="containRatingJoint">
                    <button class="ratingArt">
                        <img class="ratea" src="{beat.artworkUrl}" alt="{beat.beatTitle}">
                    </button>
                    <div>
                        <p>{beat.beatTitle}</p>
                        <p style="font-size: 8pt; opacity: .7;">
                            {beat.bpm} BPM - {beat.key} {beat.mode}
                        </p>
                    </div>
                </div>
            </div>
            ...
            {#if $isUpdatingBeatFromModal}
                ...
                <div class="wrapNoteLoader" in:fade={{duration: 300}}>
                    <p style="font-size: 9pt; opacity: .5; margin-bottom: 20px; text-align: center;">Updating Beat</p>
                    <div class="wl">
                        <AudioLoader backgroundColor={'transparent'}></AudioLoader>
                    </div>
                </div>
